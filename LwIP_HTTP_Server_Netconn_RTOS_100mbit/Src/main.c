		...
		if (err == ERR_OK) {
			...
			while ((err = netconn_recv(newconn, &buf)) == ERR_OK) {
				...
				if ((len >= 5) && (strncmp(data, "ttuzz", 5) == 0)) {
					while (err == ERR_OK) {
						err = netconn_write_partly(newconn, buffer, lenlen,	NETCONN_NOCOPY, NULL);
					}
				} else {
					/*printf("Recved\n");*/
					/*do {
...
static void StartThread(void const * argument)
{
  ...
  http_server_netconn_init();
	sys_thread_new("tcpecho_thread", tcpecho_thread, NULL,(configMINIMAL_STACK_SIZE * 3), osPriorityAboveNormal);
  for( ;; )
  {
